...
// Content retrieval:  https://192.168.0.10/incoming/Media/<file name> returns the requested file, be it HTML or other.
	...
	switch (req.params[0].slice(req.params[0].lastIndexOf('.'))) {
		...
		res.sendFile(req.params.path + req.params[0], options, function (err) {
			if (err) {
				...
				wss.broadcast('synchronizecontent:' + req.params[0]);
			}
		});
	}
});
...
server.listen(3000, function() {
	...
	if (chromex86) {
		exec('"C:/Program Files (x86)/Google/Chrome/Application/chrome.exe" --kiosk --app-auto-launched 192.168.0.10:3000', (err, stdout, stderr) => {
			if (err) {
		...
		exec('"C:/Program Files/Google/Chrome/Application/chrome.exe" --kiosk --app-auto-launched 192.168.0.10:3000', (err, stdout, stderr) => {
			if (err) {
				debugLog(ERROR_DEBUG, 'Error in exec(): ' + err);
				return;
			}
